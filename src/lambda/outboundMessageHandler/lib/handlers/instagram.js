...
const sendMessage = async (facebookId, message) => {
  ...
  log.debug('Send Instagram Message body', body);
  ...
  const result = await new Promise((resolve, reject) => {
    ...
    req.on('error', (err) => {
      log.error('Error sending Instagram message', err);
      reject(err);
    });
  ...
  log.debug('Send Instagram Message result', result);

  if (resultObj.error !== undefined) {
    log.error('Error sending Instagram message', resultObj);
    return false;
  }
