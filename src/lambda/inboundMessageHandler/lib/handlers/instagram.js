...
const CHANNEL_TYPE = 'INSTAGRAM';
const AWS = require('aws-sdk');
const secretManager = new AWS.SecretsManager();
const handler = async (messagePayloadString) => {
  log.debug('Instagram message handler');
...
const processMessagePayload = async (messagePayload) => {
  ...
  for (let i = 0; i < messagePayload.entry.length; i++) {
    ...
    if (entry.messaging === undefined) {
      log.warn(
        'Instagram "standby" webhook event is not supported.  Unsubscribe from this webhook event to reduce traffic.'
      );
      continue;
    }
    ...
    for (let j = 0; j < entry.messaging.length; j++) {
      ...
      if (messaging.message === undefined) {
        log.warn(
          'Instagram "read" webhook event is not supported.  Unsubscribe from this webhook event to reduce traffic.'
        );
        continue;
      }
...
const processMessage = async (message, participant) => {
  ...
  if (message.is_echo == true) {
    log.warn(
      'Instagram "message_echoes" webhook event is not supported.  Unsubscribe from this webhook event to reduce traffic.'
    );
    return;
  }
...
const validateRequest = async (request) => {
  ...
  if (appSecret === null) {
    log.error('Instagram Secret not found.  Cannot process record.');
    return false;
  }
  ...
  if (requestHash === payloadHash) {
    log.debug('Instagram Request Validation - Hash match');
    return true;
  } else {
    log.debug('Instagram Request Validation - Hash does not match');
    return false;
  }
};
