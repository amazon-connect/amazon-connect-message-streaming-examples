...
const handler = async (messagePayloadString) => {
  log.debug('LINE message handler');
...
const validateRequest = async (request) => {
  ...
  if (appSecret === null) {
    log.error('LINE Secret not found.  Cannot process record.');
    return false;
  }
  ...
  if (requestHash === payloadHash) {
    log.debug('LINE Request Validation - Hash match');
    return true;
  } else {
    log.debug('LINE Request Validation - Hash does not match');
    return false;
  }
};
